...
from mpscenes.obstacles import SphereObstacle
...
class Simulator:
    ...
    def _initalize_environment(self, cfg) -> UrdfEnv:
        ...
        # boxObst = SphereObstacle(name="simpleBox", content_dict=obst1Dict)
        ...
        #boxObst = DynamicSphereObstacle(name="simpleBox", content_dict=obst1Dict)
        boxObst2 = SphereObstacle(name="simpleBox", content_dict=obst2Dict)
        env.add_obstacle(boxObst2)
        ...
        env.add_debug_shape(position=[2.7,0.0,0.0],orientation=[0.0,0.0,0.0],size=[0.5,0.2],shape_type="cylinder",rgba_color=[1.0,0.0,0.0,0.3],)
        # Add the sensor
        sensor = FullSensor(
            goal_mask=["position"],
